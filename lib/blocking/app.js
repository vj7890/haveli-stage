...
/** A starting skeleton: one section, a handful of cues. Load the real sheet with "Load cue sheet". */
const SEED = [['SHOW', 'Programme', [['1', 'Opening', '', '', '', ''], ['2', '', '', '', '', ''], ['3', '', '', '', '', ''], ['4', '', '', '', '', ''], ['5', '', '', '', '', '']]]]
...
const seedSections = (sheet) => sheet.map(([code, title, cues]) => ({ id: uid('sec_'), code, title, script: '', cues: cues.map(cueFrom) }))
const blankShow = () => ({ v: 4, name: 'Haveli', venue: null, roles: DEFAULT_ROLES.map((r) => ({ ...r })), sections: seedSections(SEED) })
function migrate(d) {
...
export function mountBlocking(root) {
  ...
  $('#btnSetup').onclick = () => {
    modal('Hall seating', 'Rows of chairs facing the stage: seat pitch, row pitch, sections across, bands of rows. This is a placeholder layout until the real seating plan is known — replace the numbers and every marker stays put. Set "seats" to false for an open floor.',
  ...
  $('#btnPng').onclick = () => {
    const sc = scene(), which = surface
    modal('Export PNG', 'Full plan, or exactly what you can see now.', '', [
      { label: 'Whole plan', fn: () => { closeModal(); const v = fullView(which), w = 3000, h = Math.round((w * v.h) / v.w); rasterise(plainSVG(which, sc, cueTitle()), w, h).then((b) => { download(b, `${safeName(section().code)}_cue${safeName(sc.no)}_${which}.png`); toast('Saved') }).catch((e) => toast(e.message)) } },
      { label: 'Current view', key: true, fn: () => { closeModal(); const v = VIEWS[which] || fullView(which); const k = sel.itemId; sel.itemId = null; const svg = buildSurface(which, sc, null, v, cueTitle()).svg; sel.itemId = k; const w = 2600, h = Math.round((w * v.h) / v.w); rasterise(svg, w, h).then((b) => { download(b, `${safeName(section().code)}_cue${safeName(sc.no)}_${which}_view.png`); toast('Saved') }).catch((e) => toast(e.message)) } }])
  }
  $('#btnPrint').onclick = () => {
    const only = section()
    const run = (secs) => {
  ...
  /* ---- published images (Supabase Storage) ---------------------------------------- */
  const BUCKET = 'plans'
  async function uploadPNG(path, blob) {
    const cfg = backend && backend.cfg
    if (!cfg) throw new Error('not connected to Supabase')
    const H = { apikey: cfg.key, 'x-upsert': 'true', 'Content-Type': 'image/png', 'Cache-Control': 'max-age=60' }
    if (!/^sb_/.test(cfg.key)) H.Authorization = 'Bearer ' + cfg.key
    const r = await fetch(`${cfg.url}/storage/v1/object/${BUCKET}/${path}`, { method: 'POST', headers: H, body: blob })
    if (!r.ok) throw new Error('HTTP ' + r.status + ' — ' + (await r.text()).slice(0, 160))
    return `${cfg.url}/storage/v1/object/public/${BUCKET}/${path}`
  }
  const imgPath = (sec, q, surf) => `${safeName((backend && backend.cfg && backend.cfg.id) || DOC_ID)}/${safeName(sec.code || 'sec')}-cue${safeName(q.no)}-${surf}.png`
  let pubQueue = new Set(), pubTimer = null, publishing = false
  async function publishCues(list, onStep) {
    const v = Date.now(); let done = 0, failed = 0, firstErr = ''
    for (const [sec, q] of list) {
      const head = `${sec.code || ''}${sec.code ? ' · ' : ''}cue ${q.no}  —  ${q.title || ''}`
      try {
        const fv = (surf) => { const vw = fullView(surf); return [1600, Math.round((1600 * vw.h) / vw.w)] }
        const [sw, sh] = fv('stage'), [hw, hh] = fv('hall')
        const su = await uploadPNG(imgPath(sec, q, 'stage'), await rasterise(plainSVG('stage', q, head), sw, sh))
        const hu = await uploadPNG(imgPath(sec, q, 'hall'), await rasterise(plainSVG('hall', q, head), hw, hh))
        q.img = { stage: su, hall: hu, v }; done++
      } catch (e) { failed++; if (!firstErr) firstErr = e.message }
      onStep(done, failed, list.length)
    }
    const was = publishing; publishing = true; markDirty(); publishing = was
    return { done, failed, firstErr, v }
  }
  function sheetRows(live) {
    const L = ['Section\tCue\tItem\tStage plan\tHall plan']
    const cell = (u, v) => (live ? `=IMAGE("${u}?t="&TEXT(NOW(),"yyyymmddhh"))` : `=IMAGE("${u}?v=${v || 0}")`)
    show.sections.forEach((sec) => sec.cues.forEach((q) => { if (q.img) L.push([sec.code, q.no, q.title, cell(q.img.stage, q.img.v), cell(q.img.hall, q.img.v)].join('\t')) }))
    return L.join('\n')
  }
  function queuePublish() {
    if (!show.autoPublish || !backend || publishing) return
    pubQueue.add(section().id + '|' + scene().id)
    clearTimeout(pubTimer); pubTimer = setTimeout(flushPublish, 8000)
  }
  async function flushPublish() {
    if (publishing || !pubQueue.size) return
    const list = []
    pubQueue.forEach((k) => { const [si, qi] = k.split('|'); const sec = show.sections.find((x) => x.id === si); const q = sec && sec.cues.find((x) => x.id === qi); if (q) list.push([sec, q]) })
    pubQueue.clear(); if (!list.length) return
    publishing = true; setSync('publishing')
    try { await publishCues(list, () => {}); setSync('saved') } catch { setSync('publish failed') }
    publishing = false
  }
  const STORAGE_SQL = "insert into storage.buckets (id, name, public)\nvalues ('plans', 'plans', true)\non conflict (id) do update set public = true;\n\ncreate policy \"plans read\" on storage.objects\n  for select using (bucket_id = 'plans');\n\ncreate policy \"plans write\" on storage.objects\n  for insert with check (bucket_id = 'plans');\n\ncreate policy \"plans replace\" on storage.objects\n  for update using (bucket_id = 'plans') with check (bucket_id = 'plans');"
  $('#btnImages').onclick = () => {
    if (!backend) { modal('Publish images', 'Publishing needs the Supabase connection — open Share and connect first. Export PNG still works without it.', '', [{ label: 'Close', fn: closeModal }]); return }
    const sec = section(), nAll = show.sections.reduce((a, x) => a + x.cues.length, 0), published = show.sections.reduce((a, x) => a + x.cues.filter((q) => q.img).length, 0)
    const body = '<p class="sub">Renders both plans for each cue and puts them at a fixed web address, so the master sheet or the script doc can pull them in. Run it once now and again whenever the blocking changes. The doc script looks for <code>…-stage.png</code> and <code>…-hall.png</code>.<br><br><b>First time only:</b> paste the block below into the Supabase SQL Editor and press Run. It makes the public image folder.</p>' +
      `<pre>${esc(STORAGE_SQL)}</pre>` +
      `<label class="chk"><input type="checkbox" id="autoPub"${show.autoPublish ? ' checked' : ''}> Keep images up to date automatically (republishes a cue 8 seconds after anyone stops editing it)</label>` +
      `<div id="pubStat" class="mono">${published} of ${nAll} cues currently published</div>`
    const run = (list) => {
      const stat = $('#pubStat')
      publishCues(list, (d, f, t) => { stat.textContent = `published ${d} of ${t}${f ? '  ·  ' + f + ' failed' : ''}` }).then((r) => {
        if (r.failed && r.done === 0) { stat.textContent = 'nothing published — ' + r.firstErr; return }
        renderScenes(); stat.innerHTML = `done: ${r.done} published${r.failed ? ', ' + r.failed + ' failed (' + esc(r.firstErr) + ')' : ''}. Use <b>Copy rows</b> below, then paste into the master sheet.`
      })
    }
    modal('Publish images', '', body, [
      { label: 'Copy the SQL', fn: () => copyText(STORAGE_SQL) },
      { label: 'This cue', fn: () => run([[sec, scene()]]) },
      { label: `This section (${sec.cues.length})`, fn: () => run(sec.cues.map((q) => [sec, q])) },
      { label: `Every cue (${nAll})`, fn: () => run(show.sections.flatMap((x) => x.cues.map((q) => [x, q]))) },
      { label: 'Copy doc markers', fn: () => copyText(`[[plan:${imgPath(section(), scene(), 'stage')}]]\n[[plan:${imgPath(section(), scene(), 'hall')}]]`) },
      { label: 'Copy rows (fixed)', fn: () => { const t = sheetRows(false); if (t.split('\n').length < 2) { toast('Publish something first'); return } copyText(t) } },
      { label: 'Copy rows (self-refreshing)', key: true, fn: () => { const t = sheetRows(true); if (t.split('\n').length < 2) { toast('Publish something first'); return } copyText(t) } },
      { label: 'Close', fn: closeModal }])
    const cb = $('#autoPub'); if (cb) cb.onchange = (e) => { show.autoPublish = e.target.checked; markDirty() }
  }
  ...
  let saveTimer = null
  function markDirty() {
    dirty = true; setSync('unsaved'); queuePublish()
  ...
  return () => {
    ...
    timers.forEach(clearInterval); clearTimeout(saveTimer); clearTimeout(pubTimer)
    if (dirty) saveLocal()
  }
}
